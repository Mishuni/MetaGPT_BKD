...
def parse_score(text: str) -> Union[int, float, None]:
    # pattern = r"[-+]?\d*\.?\d+"  # Matches integers or floats
    match = re.search(r'-+\s*([\d.]+)\s*-+', text, re.DOTALL)
...
def parse_recipient(text):
    ...
    pattern = r"## Send To:\s*([A-Za-z]+)\s*?" # **Send To:**  
    recipient = re.search(pattern, text)
    if recipient:
        return recipient.group(1)
    ...
    for line_idx, line in enumerate(text_lines):
        logger.info(f"{line_idx} :: {line=}")
        if line_idx<(len(text_lines)-1) and "Send To".upper() in line.upper():
            recipient = text_lines[line_idx+1]
            return recipient
    return ""
