...
def _repair_llm_raw_output(output: str, req_key: str, repair_type: RepairType = None) -> str:
    ...
    for repair_type in repair_types:
        ...
        # elif repair_type == RepairType.ALPHABET:
        #     ## for alphabet format (Upperbound, Lowerbound, etc.)
        #     output = repair_key_alphabet(output, req_key)
    return output
...
def extract_content_from_output(content: str, right_key: str = "[/CONTENT]"):
    ...
    raw_content = raw_content.split("</think>")[-1].strip()  # <think> 부분 제거
    # print(raw_content)
    # exit(1)
    pattern = r"\[CONTENT\]([\s\S]*)\[/CONTENT\]"
    new_content = re_extract_content(raw_content, pattern)

    if not new_content.startswith("{") or not new_content.strip().startswith("{"):
