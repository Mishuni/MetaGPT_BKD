...
class ContextMixin(BaseModel):
    ...
    def llm(self) -> BaseLLM:
        ...
        # logger.info(f"class:{self.__class__.__name__}({self.name}), llm: {self._llm}, llm_config: {self._llm_config}")
        # logger.info(f"llm: {self.private_llm}, llm_config: {self.config.llm}")
        if not self.private_llm:
            logger.info(f"llm not set, init from config: {self.config.llm}")
            self.private_llm = self.context.llm_with_cost_manager_from_llm_config(self.config.llm)
        # print(self.private_llm)
