...
from metagpt.provider.general_api_base import OllamaResponse
from metagpt.provider.general_ollama_local_requestor import GeneralOllamaLocalRequestor
...
class OllamaMessageChat(OllamaMessageBase, metaclass=OllamaMessageMeta):
    ...
    def apply(self, messages: list[dict]) -> dict:
        ...
        sends = {"model": self.model, "messages": messages}
...
@register_provider(LLMType.OLLAMA_LOCAL)
class OllamaLLMLOCAL(BaseLLM):
    """
    ...
    def __init__(self, config: LLMConfig):
        ...
        self.client = GeneralOllamaLocalRequestor(base_url=config.base_url)
        # ollama.create(model='example', from_='llama3.2', system="You are Mario from Super Mario Bros.")
        # self.client = ollama
        self.config = config
        self.http_method = "post"
        self.use_system_prompt = True # False
    ...
    async def _achat_completion(self, messages: list[dict], timeout: int = USE_CONFIG_TIMEOUT) -> dict:
        ...
        # exit(1)
        # logger.info(json.dumps(self.ollama_message.apply(messages=messages), indent=4))
        # print(self.ollama_message)
        # exit(1)
        ...
        elif isinstance(resp, OllamaResponse):
            return self._processing_ollama_local_response(resp)
        else:
            raise ValueError
    ...
    def _processing_ollama_local_response(self, resp: OllamaResponse):
        return resp
...
class OllamaGenerate(OllamaLLMLOCAL):
    @property
    def _llama_api_inuse(self) -> OllamaMessageAPI:
...
class OllamaEmbeddings(OllamaLLMLOCAL):
    @property
    def _llama_api_inuse(self) -> OllamaMessageAPI:
