...
from metagpt.prompts.ma_sec_prompts import NEW_REQ_TEMPLATE_FOR_WRITE_TASK_WITH_PROJECT_MANAGER
...
class WriteTasks(Action):
    ...
    async def _merge(self, system_design_doc, task_doc) -> Document:
        context = NEW_REQ_TEMPLATE_FOR_WRITE_TASK_WITH_PROJECT_MANAGER.format(context=system_design_doc.content, old_task=task_doc.content)
    ...
    async def _update_requirements(self, doc):
        ...
        if len(packages) == 0 or (len(packages) == 1 and "No third-party dependencies required" in packages):
            return
        if (len(packages) == 1 and "No third-party packages required" in packages):
            return
        if (len(packages) == 1 and "No third-party Python packages require" in packages):
            return
        if (len(packages) == 1 and "No t" in packages):
            return
        # if ("No " in packages[0]):
        #     return
