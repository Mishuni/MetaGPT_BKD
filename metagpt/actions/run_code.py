...
from metagpt.prompts.ma_sec_prompts import RUN_CODE_FOR_QA, RUN_CODE_BACKDOOR_FOR_QA
...
class RunCode(Action):
    ...
    async def run(self, *args, **kwargs) -> RunCodeResult:
        ...
        if backdoor:
            prompt = RUN_CODE_BACKDOOR_FOR_QA.format(context=context,\
                source_file_path=fake_root + "/" + code_doc.root_relative_path,workspace=fake_root,
                test_file_name=self.i_context.test_filename)
        else:
            prompt = RUN_CODE_FOR_QA.format(context=context)
    ...
    def _install_dependencies(working_directory, env):
        try:
            ...
            logger.error(f"Error while installing requirements: {e}")
