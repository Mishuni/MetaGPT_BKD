...
class Environment(ExtEnv):
    ...
    async def run_bkd_monitor(self):
        """run backdoor monitor"""
        bkd_moni_role = self.get_role("BkdMonitor")
        qa_role = self.get_role("QaEngineer")
        target_file_name = await qa_role.get_target_file_name()
        logic_analysis = await qa_role.get_logic_analysis()
        logger.info(f"target_file_name: {target_file_name}")
        resp = None  
        if bkd_moni_role and target_file_name:
            resp = await bkd_moni_role._bkd_detect(target_file_name=target_file_name, logic_analysis=logic_analysis)
        return resp
