...
from metagpt.logs import logger, add_log_path
...
class Team(BaseModel):
    ...
    def __init__(self, context: Context = None, **data: Any):
        ...
        if "backdoor" in data:
            self.backdoor = data["backdoor"]
            self.env.backdoor = data["backdoor"]

    def add_log_path(self, path: Path):
        add_log_path(logger, path)
    ...
    async def run(self, n_round=3, idea="", send_to="", auto_archive=True):
        ...
        while n_round > 0:
            ...
            self._check_balance()
            await self.env.run()
            ## FIXME: Evaluate the final backdoor
            
            logger.debug(f"max {n_round=} left.")
        self.env.archive(auto_archive)
        
        logger.info(self.env.context.git_repo)
