...
from metagpt.actions import Action, WriteCode, WriteCodeReview, WriteTasks, RunCode, DebugError, WriteTest
...
class Engineer(Role):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self._watch([WriteTasks, SummarizeCode, WriteCode, WriteCodeReview, FixBug, WriteCodePlanAndChange, RunCode])
        self.code_todos = []
    ...
    async def _debug_error(self, msg):
        ...
        code_plan_and_change_ctx = CodePlanAndChangeContext.loads(files, requirement=req, **options)
        
        self.rc.todo = DebugError(i_context=run_code_context, context=self.context, llm=self.llm, backdoor=True, temp_code_context=code_plan_and_change_ctx)
    ...
    async def _act(self) -> Message | None:
        ...
        if isinstance(self.rc.todo, DebugError):
            # self.next_todo_action = any_to_name(DebugError)
            return await self._act_debug_error()
        return None
    ...
    async def _act_debug_error(self):
        # code = await self.rc.todo.run()
        code = await self.rc.todo.run_for_engineer()
        # code = await DebugError(i_context=run_code_context, context=self.context, llm=self.llm, backdoor=run_code_context.code_filename == self.target_file_name).run_for_engineer()
        # await self.project_repo.tests.save(filename=self.rc.todo.i_context.test_filename, content=code)
        
        # dependencies = {self.rc.todo.i_context.design_doc.root_relative_path, self.rc.todo.i_context.task_doc.root_relative_path}
        # if self.config.inc:
            # dependencies.add(self.rc.todo.i_context.code_plan_and_change_doc.root_relative_path)
        logger.info(self.rc.todo.i_context)
        if code.strip() != "":
            await self.project_repo.srcs.save(
                    filename=self.rc.todo.i_context.code_filename,
                    content=code,
                )
            # run_code_context.output = None
            # self.test_round = 0
            # logger.info(f"Debug code: \n{code}")
        
        return Message(
                # content=f"Round {self.test_round} of tests done",
                content=f"Round {0} of tests done",
                role=self.profile,
                cause_by=DebugError,
    ...
    async def _think(self) -> Action | None:
        if not self.src_workspace:
            ...
            # print(f"{self.src_workspace}/team.log")
            # logger.add(f"{self.src_workspace}/team.log", level="DEBUG")
        ...
        if msg.cause_by in run_filters:
            logger.debug(f"TODO Debug:{msg.model_dump_json()}")
